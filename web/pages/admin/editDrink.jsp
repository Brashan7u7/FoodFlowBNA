...
<html>
    ...
    <body>
        ...
        <div style="margin-left: 100px; padding: 20px; min-width: 100vh;">
            ...
            %>
            <form action="${pageContext.request.contextPath}/editDrink" method="post"> 
                ...
                <div class="row">
                    <div class="col-md-5 offset-md-4">
                        <button type="button" onclick="actualizarBebida(<%=id%>)" class="btn btnUpdate">Actualizar</button>
                        <button type="button" class="btn btnDelete" onclick="eliminarBebida(<%=id%>)">
                            Eliminar
                        </button>
                    </div>
                </div>
        ...
        <script>
            ...
            function actualizarBebida(id) {
                ...
                if (!nombre || !descripcion || !precio_unitario) {
                    mostrarModal("error", "Error al actualizar la bebida. Verifica los datos ingresados.");
                    return;
                }
                ...
                .then(() => {
                    mostrarModal("success", "¡Bebida actualizada con éxito!");
                    setTimeout(() => {
                        window.location.href = "<%= request.getContextPath()%>/bebidas";
                    }, 2000);
                })
                .catch(error => {
                    console.error('Error:', error);
                    mostrarModal("error", "Hubo un problema al actualizar la bebida. Inténtalo nuevamente.");
                });
            }
            ...
            function eliminarBebida(id) {
                if (confirm("¿Estás seguro de que quieres eliminar esta bebida?")) {
                    fetch(`editDrink?id=` + id, {
                        ...
                        if (response.ok) {
                            mostrarModal("success", "¡Bebida eliminada con éxito!");
                            setTimeout(() => {
                                window.location.href = "<%= request.getContextPath()%>/bebidas";
                            }, 1000);
                        } else {
